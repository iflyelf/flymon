// Package edge 提供 flymon-edge 服务的初始化逻辑。
//
// edge 是边缘节点服务，包含告警引擎 + 推送网关（离中心服务独立运行）。
// 上游 edge Initialize 位于 cmd/edge/edge.go 的 package main 中无法直接导入，
// 因此这里复刻其逻辑，所有依赖均从 upstream 可导出包导入，保证可跟随上游升级。
package edge
...
// Initialize 初始化 edge 服务（与上游 cmd/edge/edge.go:Initialize 逻辑一致）。
// 返回的 cleanup 函数用于优雅关闭。
